...
通过使用，我们可以看到，我们传给GTabs信息，除了close-able和add-able

以及双绑数据-当前active的tab，还有插槽信息，没了。

但是我们的GTabs组件，需要去渲染Tab可选栏，要可以渲染添加的栏，删除已存在的栏

所以需要的数据，应该借助插入GTabs组件内的GTabItem来获取。
...
可以看到code部分，我们直接就是provide，把需要GTab-item传给我们的、props传给我们需要给Item的

都provide，之后每次在GTabs组件内部进行渲染GTabItem组件时（GTabItem --> Tabs对应渲染的内容）

就会把自己的数据push进 GTabs provide的数组，然后GTabs再根据这个数组

去进行添加、删除、渲染可以被点击的Tab。

因为 GTabs的 GTabItem 如果是一开始 插槽插入，

```html
<GTabItem>内容</GTabItem>
```

显示GTabItem插槽内容就行，而如果是自己在GTabs添加的新Tab

那么content需要自己进行定义。

（注意：改变、切换当前active的tab，需要改变provide给GTabItem的cur_active_tab信息，也需要emit出去，因为双绑了）
...
对于依赖库的使用，看base-popover的code部分

popover组件内部也是对base-popover组件的调用
...
export const popoverProps = {
  ...
  // base-popover四个props，这个title是popover特有的，
  // 插槽，插入base-popover的时候用
  title: {
    type: String,
    default: ''
  },
  ...
  setup(props: BasePopoverProps, { slots, attrs }) {
    ...
    const updatePosition = () => {
      ...
      computePosition(hostDom.value, overlayRef.value, {
        ...
        // 上面传入middleware就可以拿到参数：middlewareData, placement
        // 拿到浮动元素的x和y，进行设置（css进行了绝对定位）
        /*
            let target = { a: 1, b: 2 };
            let source = { b: 4, c: 5 };
        ...
        */
        ...
        if (showArrow.value) {
          /*
          	arrowX和arrowY是中间件的箭头相对于浮动框的位置，0和0就在左上角
          	这里因为箭头向上，且是bottom，而不是bottom-start/end，所以是一个
          	left:80多px，top:undefined
          */
          const { x: arrowX, y: arrowY } = middlewareData.arrow!
          // 箭头的偏移量 和 border哪条边设置透明，需要一个映射表
          /* 获取当前所在边 (placement拿到当前定位的值，用外面placement.value也可)
          	这里.then拿到的值 类型定义如下：一共12种
            ...
            比如props默认placement就是bottom，我现在拿到了 bottom*/
          const currentSide = placement.split('-')[0] // currentSide也是bottom
          const staticSide = {
          ...
          /*
          	我现在currentSide为bottom，气泡窗口在宿主dom下面，
          	在顺时针旋转45deg（scss种旋转了）后，箭头应该是指向的上面的，那么
          	应该是右边框和下边框是一个透明的。我们可以拿到currentSide，但是要如何拿到right呢？
          */
          // 算法
          ...
          Object.assign(arrowRef.value.style, {
            left: arrowX + 'px',
            top: arrowY + 'px',
            [staticSide!]: '-4px', // 看scss，宽高均为8px，往下走top为-4
    ...
    watch(
      ...
      newVal => {
        if (newVal) {
          ...
          hostDom.value &&
            mutationObserver.observe(hostDom.value, { attributes: true }) // 启动对宿主的监听
    ...
    // popover频繁的弹出和销毁
    onUnmounted(() => {
      mutationObserver.disconnect() // 结束对宿主的监听
      window.removeEventListener('resize', updatePosition)
      window.removeEventListener('scroll', updatePosition)
    })
...
首先，props传入的Component作为包裹列表的标签，绑定上ref

在挂载后，高度设置为视口高度(这也就是为什么，在使用时，需要在整个VirtualList组件外面，包裹一个有高度的div的原因)

可视区列表元素数量，通过刚刚拿到的视口高度 ，除以每个列表项的高度

向上取整，因为显示一点，也是算显示，也要渲染。

然后再在Component标签上绑定好scroll事件，设置好overflow：hidden

在这个Component标签内，有两个div子标签，第一个是背景板，高度就是 子元素高度 * 渲染数组子元素个数

第二个就是在当前的视口高度下，显示的所有的标签（visibleData数组.map）。

visibleData数组是computed出来的，依赖当前视口第一个列表项在源数组中的index。

每次一滚动，会都去通过 scrollTop和itemHeight，向下取整

拿到当前显示的第一个列表项，在data数组中的index，改变，触发visibleData的重计算

与此同时，在滚动事件中还通过 `offsetY.value = scrollTop - (scrollTop % itemHeight.value)`让每次都滚动一个列表项的高度

（visibleData的计算中，需要考虑下数组越界的问题，用下 `Math.min`）
...
简单看看使用，然后看tree组件props，再看utils，再看tree组件的code，

可以看到我们作为prop传入的data，是直接传入调用了hook --- useTree。

看hook --- use-tree（含有其他hook的一些引用和注释）。

看完hooks部分，再回来看使用，先看前两种，没有使用GTree插槽的部分

再去看code，render函数中，可以知道，设置了高度，就给他开启虚拟滚动，外面套个有高度的div，好让他拿视口高度
...
先简单将tree组件的使用分为两类，有用slot（icon、content、loading）的，和没用slot的

tree-node插槽也是这三个（因为本质上就是传给tree-node的，tree只是做了个判断，这边slot没传，他给tree-node插槽默认值）
...
export function useTree(
  ...
  // 遍历的时候都传了lazyLoad，但是实际上用到的只有useExpand的hook
  // 结合useExpand看use-lazy-load的hook，其实就是判断节点innerNode是否是叶子、是否展开
  // 符合条件，就把当前找到然后和封装好的回调dealLazyChildNodeCB，一起emit出去
  /*
      innerNode.loading = true
      emit('lazy-load', node, dealLazyChildNodeCB)
  */
...
一开始的数据就是一个ITreeNode类型数组，他的children是一个数组，保存了他的子节点，类型还是ITreeNode

这里要把他扁平化，并且把节点都处理为 IFlatTreeNode类型（extend ITreeNode，多了parentId、level、isLeaf属性）
